...
const Portfolio = () => {
  ...
  const Card = ({ thumbnail, title, description }) => (
    <div className="projectIcons">
      <div className="column">
        <img src={thumbnail} alt="Description" style={{ maxWidth: '350%', maxHeight: '200px' }} />
        <h2>{title}</h2>
        <p>{description}</p>
      </div>
    </div>
  ...
  return (
    <div className="grid-container-template" style={{ display: 'flex', flexDirection: 'row', gridGap: '20px' }}>
      {/* Render the first 3 Card components */}
      <div className="flex-container" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        <h1 className="mainText">Hello</h1>
        {projects.slice(0, 3).map((project, index) => (
          <Card key={index} thumbnail={project.thumbnail} title={project.title} description={project.description} />
        ))}
      </div>
      {/* Render the last 3 Card components */}
      <div className="flex-container" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        {projects.slice(3).map((project, index) => (
          <Card key={index + 3} thumbnail={project.thumbnail} title={project.title} description={project.description} />
        ))}
      </div>
    </div>
  );
